...
function getYears(fifaData, index) {
    const fifaYears
    for(index = 0; index.length - 1; index++){
        return fifaData.Year
    }
    fifaYears.push(index, fifaData.Year)
};

console.log(getYears(index, fifaYears));

/* Task 5: Implement a higher-order function called `getWinners`, that accepts the 
callback function `getFinals()` and determine the winner (home or away) of each
...
the following parameters and returns a set of strings "In {year}, {country} won the 
world cup!" 
